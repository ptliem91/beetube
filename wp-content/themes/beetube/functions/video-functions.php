 ...
 * @since 1.0
 */
function jtheme_video($post_id, $autoplay = false) {
	$file = get_post_meta($post_id, 'jtheme_video_file', true);
	$files = !empty($file) ? explode("\n", $file) : array();
	$url = trim(get_post_meta($post_id, 'jtheme_video_url', true));
	$code = trim(get_post_meta($post_id, 'jtheme_video_code', true));
...
*/
function bee_has_video( $post_id ){
	ob_start();
	$chk = '';
	global $post;
	$postID = $post->ID;
	...
	jtheme_video( $postID, get_option('jtheme_single_video_autoplay') );
