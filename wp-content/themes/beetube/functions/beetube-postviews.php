...
add_action('admin_menu', 'register_beetube_post_views_page');

function register_beetube_post_views_page() {
	add_submenu_page( 'options-general.php', 'BeeTube Post Views', 'BeeTube Post Views', 'manage_options', 'beetube-post-views', 'beetube_post_views_callback' ); 
}

function beetube_post_views_callback() {
	...
	function views_default_templates(template) {
		...
		switch(template) {
			...
			case 'most_viewed':
				...
				default_template = "<li>%POST_THUMB%<a href=\"%POST_URL%\">%POST_TITLE%</a> views %VIEW_COUNT%</li>";
				break;
		}
...
function wp_postview_cache_count_enqueue() {
	...
	if (!wp_is_post_revision($post) && (is_single() || is_page())) {
		...
		if ($should_count && defined('WP_CACHE') && WP_CACHE) {
			...
			wp_enqueue_script('wp-postviews-cache', get_template_directory_uri().'/js/postviews-cache.js', array('jquery'), '1.64', true);
	...
	function get_most_viewed($mode = '', $limit = 10, $chars = 0, $display = true) {
		...
		if($most_viewed) {
			...
			foreach ($most_viewed as $post) {
				$post_views = intval($post->views);
				$post_title = get_the_title($post);
				$item_format = is_video($post) ? 'video' : 'post';
				...
				if ( has_post_thumbnail($post->ID)) {
					$thumb = '';
				  $thumb = '<div class="views-thumb"><a class="item-'.$item_format.'" href="' . get_permalink( $post->ID ) . '" title="' . esc_attr( $post->post_title ) . '">'.get_the_post_thumbnail($post->ID, array(120,80)).'</a></div>';
				}
				else{
					$thumb = '<div class="views-thumb"><a class="item-'.$item_format.'" href="' . get_permalink( $post->ID ) . '" title="' . esc_attr( $post->post_title ) . '"><img src="'.get_template_directory_uri().'/images/nothumb.png" /></a></div>';
				}
				$post_excerpt = views_post_excerpt($post->post_excerpt, $post->post_content, $post->post_password, $chars);
				$temp = stripslashes($views_options['most_viewed_template']);
				$temp = str_replace("%VIEW_COUNT%", number_format_i18n($post_views), $temp);
				...
				$temp = str_replace("%POST_CONTENT%", $post->post_content, $temp);
				$temp = str_replace("%POST_URL%", get_permalink($post), $temp);
				$temp = str_replace("%POST_THUMB%",  $thumb, $temp);
				$output .= $temp;
			}
			}
		} else {
