...
abstract class jtheme_Panel {
	...
	function __construct() {
		...
		$this->donate_url = !empty($this->donate_url) ? $this->donate_url : 'http://joinwebs.com/donate/';
		$this->support_url = !empty($this->support_url) ? $this->support_url : 'http://joinwebs.com/support/';
		$this->translating_url = !empty($this->translating_url) ? $this->translating_url : 'http://joinwebs.com/';
	...
	function add_default_meta_boxes($meta_boxes = array()) {
		...
		if(in_array('plugin-info', $meta_boxes) && $this->plugin_file)
			add_meta_box('plugin-info-meta-box', __('Plugin Info', 'jtheme'), array(&$this, 'plugin_info_meta_box'), $page_hook, 'side' );
		
		if(in_array('theme-info', $meta_boxes))
			add_meta_box('theme-info-meta-box', __('Theme Info', 'jtheme'), array(&$this, 'theme_info_meta_box'), $page_hook, 'side' );
		
		if(in_array('like-this', $meta_boxes))
			add_meta_box('like-this-meta-box', __('Like This?', 'jtheme'), array(&$this, 'like_this_meta_box'), $page_hook, 'side' );
		
		if(in_array('need-support', $meta_boxes))
			add_meta_box('need-support-meta-box', __('Need Support?', 'jtheme'), array(&$this, 'need_support_meta_box'), $page_hook, 'side' );
		
		if(in_array('quick-preview', $meta_boxes))
			add_meta_box('quick-preview-meta-box', __('Quick Preview', 'jtheme'), array(&$this, 'quick_preview_meta_box'), $page_hook, 'side' );
	}
	...
	function plugin_info_meta_box() {
		...
		echo '<p>' . __('Name:', 'jtheme') . ' <a target="_blank" href="'.$plugin_data['PluginURI'].'"><strong>' . $plugin_data['Name'] . '</strong></a></p>';
		echo '<p>' . __('Version:', 'jtheme') . ' ' .$plugin_data['Version'] . '</p>';
		echo '<p>' . __('Author:', 'jtheme') . ' <a href="'.$plugin_data['AuthorURI'].'">' . $plugin_data['Author'] . '</a></p>';
		echo '<p>' . __('Description:', 'jtheme') . ' '. $plugin_data['Description'] . '</span></p>';
	}
	...
	function theme_info_meta_box() {
		...
		echo '<p>' . __('Name:', 'jtheme') . ' <a target="_blank" href="'.$theme_data['URI'].'"><strong>' . $theme_data['Name'] . '</strong></a></p>';
		echo '<p>' . __('Version:', 'jtheme') . ' ' .$theme_data['Version'] . '</p>';
		echo '<p>' . __('Author:', 'jtheme') . ' <a href="'.$theme_data['AuthorURI'].'">' . $theme_data['Author'] . '</a></p>';
		echo '<p>' . __('Description:', 'jtheme') . ' '. $theme_data['Description'] . '</span></p>';
	}
	
	function like_this_meta_box() {
		echo '<p>' . __('We spend a lot of effort on Free Wordpress development. Any help would be highly appreciated. Thanks!', 'jtheme') . '</p>';
		echo '<ul>';
		
		$plugin_data = get_plugin_data( trailingslashit(WP_PLUGIN_DIR) . $this->plugin_file, false);
		
		echo '<li class="link-it"><a href="' . $plugin_data['PluginURI']. '">' . __('Link to it so others can find out about it', 'jtheme') . '</a></li>';

		if( !empty($this->wp_plugin_url) )
			echo '<li class="rating-it"><a href="' . $this->wp_plugin_url . '">' . __('Give it a good rating on Wordpress.org', 'jtheme') . '</a></li>';
		
		if( !empty($this->donate_url) )
			echo '<li class="donate-it"><a href="' . $this->donate_url. '">' . __('Donate something to our team', 'jtheme') . '</a></li>';
		
		if( !empty($this->translating_url) )
			echo '<li class="trans-it"><a href="' . $this->translating_url. '">' . __('Help us translating it', 'jtheme') . '</a></li>';
	...
	function need_support_meta_box() {
		echo '<p>';
		echo sprintf(__('If you have any problems or ideas for improvements or enhancements, please use the <a href="%s">Our Support Forums</a>.', 'jtheme'), $this->support_url );
		echo '</p>';
	}
	...
	function meta_box($object, $box) {
		...
		$new_fields = jtheme_instance_fields($defaults);
		jtheme_form_fields($new_fields);
	}
	...
	function update() {
		...
		if (isset($_POST['save'])) {
			...
			do_action('jtheme_update_settings');
			do_action('jtheme_save_settings');
		...
		elseif(isset($_POST['reset'])) {
			...
			do_action('jtheme_update_settings');
			do_action('jtheme_reset_settings');
	...
	function admin_notices() {
		...
		if (!empty($_GET['updated']) && $parent_file != 'options-general.php')
			echo '<div id="message" class="updated"><p><strong>' . __('Settings Saved.', 'jtheme') . '</strong></p></div>';

		elseif (!empty($_GET['reset']))
			echo '<div id="message" class="updated"><p><strong>' . __('Settings Reset.', 'jtheme') . '</strong></p></div>';
	...
	function plugin_action_links( $actions, $plugin_file ) {
			if ( $plugin_file == $this->plugin_file && $this->settings_url)
				$actions[] = '<a href="'.$this->settings_url.'">' . __('Settings', 'jtheme') .'</a>';
	...
	function plugin_row_meta( $plugin_meta, $plugin_file ){
			if ( $plugin_file == $this->plugin_file ) {
				$plugin_meta[] = '<a href="'.$this->donate_url.'">' . __('Donate', 'jtheme') .'</a>';
				$plugin_meta[] = '<a href="'.$this->support_url.'">' . __('Support', 'jtheme') .'</a>';
			}
	...
	function theme_action_links($links) {
		 $links[] = '<a href="'.admin_url('options.php').'">' . __('Settings', 'jtheme') .'</a>';
		return $links;
	}
	...
	function screen_icon() {
		echo '<a target="_blank" href="http://joinwebs.com">' . get_screen_icon('themes') . '</a>';
	}
	...
	function page_buttons($in_top = false) {
		if(in_array('save', $this->buttons))
			echo '<input type="submit" name="save" value="'.__('Save Changes', 'jtheme').'" class="button-primary"> ';
		
		if(in_array('reset', $this->buttons))
			echo '<input type="submit" value="'.__('Reset Settings', 'jtheme').'" name="reset" class="reset button button-highlighted"> ';
		
		if(in_array('toggle', $this->buttons) && $this->layout_like != 'settings' && $in_top)
			echo '<input type="button" class="button toggel-all" value="'.__('Toggle Boxes', 'jtheme').'" />';
	}
	...
	function menu_page() {  
		...
		?>
		<div class="wrap jtheme-panel">
			<form method="post" action="">
				<?php wp_nonce_field('closejthemeostboxes', 'closejthemeostboxesnonce', false ); ?>
	...
	<div id="dashboard-widgets" class="metabox-holder <?php echo $class; ?>">
		...
		<div id="postbox-container-2" class="postbox-container" style="<?php echo $hide2; ?>">
			<?php do_meta_boxes($screen->id, 'side', null); ?>
		</div><!-- end .postbox-container -->
					
		<div id="postbox-container-3" class="postbox-container" style="<?php echo $hide3; ?>">
			<?php do_meta_boxes($screen->id, 'column3', null); ?>
		</div><!-- end .postbox-container -->
					
		<div id="postbox-container-4" class="postbox-container" style="<?php echo $hide4; ?>">
	...
	function defaults( $fields = array() ) {
		...
		foreach($fields as $box_id => $box_fields) {
			foreach(jtheme_field_options($box_fields) as $name => $field) {
				$defaults[$name] = $field;
			}
		}
	...
	function default_scripts() {
		...
		jtheme_enqueue_admin_styles();
		jtheme_enqueue_admin_scripts();
	}
...
function jtheme_register_panel($class) {
	jtheme_Panel::register($class);
}

function jtheme_unregister_panel($class) {
	jtheme_Panel::unregister($class);
}
 ...
 * Post Meta Box
 *=============================================================================*/ 
abstract class jtheme_Post_Panel {
	protected $name;
	protected $title;
	...
	function meta_box($object, $box) {
		...
		$new_fields = jtheme_instance_fields($defaults, 'post_meta');
		
		wp_nonce_field( $this->nonce_action, $this->nonce );
		echo '<div class="jtheme-panel">';
		echo jtheme_form_fields($new_fields);
		echo '</div>';
	}
	...
	function handle($post_id, $post) {
		...
		foreach ( jtheme_field_options($fields) as $name => $field) {
...
function jtheme_register_post_panel($class) {
	jtheme_Post_Panel::register($class);
}

function jtheme_unregister_post_panel($class) {
	jtheme_Post_Panel::unregister($class);
}
 ...
 * Term Meta Box
 *=============================================================================*/
abstract class jtheme_Term_Panel {
	protected $name;
	protected $title;
	...
	function meta_box($term, $taxonomy) {
		...
		$new_fields = jtheme_instance_fields($defaults, 'term_meta', $term);
...
function jtheme_register_term_panel($class) {
	jtheme_Term_Panel::register($class);
}

function jtheme_unregister_term_panel($class) {
	jtheme_Term_Panel::unregister($class);
}

/*=============================================================================*
 * User Meta Box
 *=============================================================================*/
abstract class jtheme_User_Panel {
	protected $name;
	protected $title;
	...
	function meta_box($user) {
		...
		$new_fields = jtheme_instance_fields($defaults, 'user_meta', $user);
...
function jtheme_register_user_panel($class) {
	jtheme_User_Panel::register($class);
}

function jtheme_unregister_user_panel($class) {
	jtheme_User_Panel::unregister($class);
}
 ...
 */
add_filter('set-screen-option', 'jtheme_set_screen_option_filter', 999, 3);
function jtheme_set_screen_option_filter($status, $option, $value) {
	$value = (int) $value;
